import React, { useState, useEffect } from 'react';
import { motion, AnimatePresence } from 'framer-motion';
import '../../styles/darkMode.css';

export interface SocialIconsProps {
  className?: string;
  onToggle?: (isOpen: boolean) => void;
}

export default function SocialIconsView({ 
  className = '',
  onToggle = () => {}
}: SocialIconsProps) {
  const [isOpen, setIsOpen] = useState(false);
  
  const toggleMenu = (e: React.MouseEvent) => {
    // Detener la propagación del evento para evitar conflictos
    e.stopPropagation();
    
    const newState = !isOpen;
    setIsOpen(newState);
    onToggle(newState);
    
    // Guardar el estado en localStorage para persistencia
    localStorage.setItem('socialIconsVisible', newState.toString());
    
    // Disparar un evento personalizado para notificar a otros componentes
    const event = new CustomEvent('socialIconsVisibilityChanged', { detail: { isOpen: newState } });
    document.dispatchEvent(event);
    
    // Añadir o eliminar la clase del body para manejar el posicionamiento de los iconos de navegación
    if (newState) {
      document.body.classList.add('social-menu-open');
    } else {
      document.body.classList.remove('social-menu-open');
    }
  };
  
  // Cargar el estado inicial desde localStorage
  useEffect(() => {
    const savedState = localStorage.getItem('socialIconsVisible');
    if (savedState !== null) {
      const parsedState = savedState === 'true';
      setIsOpen(parsedState);
      onToggle(parsedState);
      
      // Aplicar la clase al body según el estado inicial
      if (parsedState) {
        document.body.classList.add('social-menu-open');
      } else {
        document.body.classList.remove('social-menu-open');
      }
    }
  }, [onToggle]);
  
  const socialLinks = [
    {
      name: 'LinkedIn',
      url: 'https://www.linkedin.com/in/giancarlo-zevallos/',
      icon: (
        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="currentColor">
          <path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z" />
        </svg>
      ),
      color: '#0077B5' // Color de LinkedIn
    },
    {
      name: 'GitHub',
      url: 'https://github.com/giancarlo92',
      icon: (
        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="currentColor">
          <path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z" />
        </svg>
      ),
      color: '#333333' // Color de GitHub
    },
    {
      name: 'Twitter',
      url: 'https://twitter.com/giancarlo_92',
      icon: (
        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="currentColor">
          <path d="M24 4.557c-.883.392-1.832.656-2.828.775 1.017-.609 1.798-1.574 2.165-2.724-.951.564-2.005.974-3.127 1.195-.897-.957-2.178-1.555-3.594-1.555-3.179 0-5.515 2.966-4.797 6.045-4.091-.205-7.719-2.165-10.148-5.144-1.29 2.213-.669 5.108 1.523 6.574-.806-.026-1.566-.247-2.229-.616-.054 2.281 1.581 4.415 3.949 4.89-.693.188-1.452.232-2.224.084.626 1.956 2.444 3.379 4.6 3.419-2.07 1.623-4.678 2.348-7.29 2.04 2.179 1.397 4.768 2.212 7.548 2.212 9.142 0 14.307-7.721 13.995-14.646.962-.695 1.797-1.562 2.457-2.549z" />
        </svg>
      ),
      color: '#1DA1F2' // Color de Twitter/X
    },
    {
      name: 'YouTube',
      url: 'https://www.youtube.com/@giancarlozevallos',
      icon: (
        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="currentColor">
          <path d="M19.615 3.184c-3.604-.246-11.631-.245-15.23 0-3.897.266-4.356 2.62-4.385 8.816.029 6.185.484 8.549 4.385 8.816 3.6.245 11.626.246 15.23 0 3.897-.266 4.356-2.62 4.385-8.816-.029-6.185-.484-8.549-4.385-8.816zm-10.615 12.816v-8l8 3.993-8 4.007z"/>
        </svg>
      ),
      color: '#FF0000' // Color de YouTube
    },
  ];

  return (
    <>
      <button
        onClick={(e) => toggleMenu(e)}
        className={`social-toggle-button ${className}`}
        aria-label={isOpen ? "Ocultar redes sociales" : "Mostrar redes sociales"}
      >
        {isOpen ? (
          <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="currentColor">
            <path d="M19 6.41L17.59 5 12 10.59 6.41 5 5 6.41 10.59 12 5 17.59 6.41 19 12 13.41 17.59 19 19 17.59 13.41 12z"/>
          </svg>
        ) : (
          <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="currentColor">
            <path d="M18 16.08c-.76 0-1.44.3-1.96.77L8.91 12.7c.05-.23.09-.46.09-.7s-.04-.47-.09-.7l7.05-4.11c.54.5 1.25.81 2.04.81 1.66 0 3-1.34 3-3s-1.34-3-3-3-3 1.34-3 3c0 .24.04.47.09.7L8.04 9.81C7.5 9.31 6.79 9 6 9c-1.66 0-3 1.34-3 3s1.34 3 3 3c.79 0 1.5-.31 2.04-.81l7.12 4.16c-.05.21-.08.43-.08.65 0 1.61 1.31 2.92 2.92 2.92 1.61 0 2.92-1.31 2.92-2.92s-1.31-2.92-2.92-2.92z"/>
          </svg>
        )}
      </button>
      
      <AnimatePresence>
        {isOpen && socialLinks.map((link, index) => (
          <motion.div
            key={`social-${index}`}
            className="navigation-tooltip"
            initial={{ opacity: 0, x: 20 }}
            animate={{ opacity: 1, x: 0 }}
            exit={{ opacity: 0, x: 20 }}
            transition={{ duration: 0.3, delay: index * 0.1 }}
            style={{
              position: 'fixed',
              right: '15px',
              top: `${125 + (index * 50)}px`
            }}
          >
            <a
              href={link.url}
              target="_blank"
              rel="noopener noreferrer"
              className="navigation-icon-item"
              aria-label={link.name}
              style={{
                backgroundColor: link.color || '#6b7280',
                color: 'white'
              }}
            >
              {link.icon}
            </a>
            <span className="tooltip-text">{link.name}</span>
          </motion.div>
        ))}
      </AnimatePresence>
    </>
  );
}
